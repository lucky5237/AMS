...
@interface User_Onrspqryoptionselfclose : BaseResponseModel
...
@property(nonatomic,strong) NSNumber *OrderSubmitStatus;//期权自对冲提交状态    '0'->已经提交;'1'->撤单已经提交;'2'->修改已经提交;'3'->已经接受;'4'->报单已经被拒绝;'5'->撤单已经被拒绝;'6'->改单已经被拒绝;
...
@property(nonatomic,strong) NSNumber *ExecResult;//自对冲结果    'n'->没有执行;'c'->已经取消;'0'->执行成功;'1'->期权持仓不够;'2'->资金不够;'3'->会员不存在;'4'->客户不存在;'6'->合约不存在;'7'->没有执行权限;'8'->不合理的数量;'9'->没有足够的历史成交;'a'->未知;
