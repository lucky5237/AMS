...
@interface User_Onrspqryinstrument : BaseResponseModel
...
@property(nonatomic,strong) NSNumber *PriceTick;//最小变动价位
...
@property(nonatomic,strong) NSNumber *InstLifePhase;//合约生命周期状态    '0'->未上市;'1'->上市;'2'->停牌;'3'->到期;
@property(nonatomic,strong) NSNumber *IsTrading;//当前是否交易
@property(nonatomic,strong) NSNumber *PositionType;//持仓类型    '1'->净持仓;'2'->综合持仓;
@property(nonatomic,strong) NSNumber *PositionDateType;//持仓日期类型    '1'->使用历史持仓;'2'->不使用历史持仓;
@property(nonatomic,strong) NSNumber *LongMarginRatio;//多头保证金率
@property(nonatomic,strong) NSNumber *ShortMarginRatio;//空头保证金率
@property(nonatomic,strong) NSNumber *MaxMarginSideAlgorithm;//是否使用大额单边保证金算法    '0'->不使用大额单边保证金算法;'1'->使用大额单边保证金算法;
@property(nonatomic,copy) NSString *UnderlyingInstrID;//基础商品代码
@property(nonatomic,strong) NSNumber *StrikePrice;//执行价
@property(nonatomic,strong) NSNumber *OptionsType;//期权类型    '1'->看涨;'2'->看跌;
@property(nonatomic,strong) NSNumber *UnderlyingMultiple;//合约基础商品乘数
@property(nonatomic,strong) NSNumber *CombinationType;//组合类型    '0'->期货组合;'1'->垂直价差BUL;'2'->垂直价差BER;'3'->跨式组合;'4'->宽跨式组合;'5'->备兑组合;'6'->时间价差组合;
