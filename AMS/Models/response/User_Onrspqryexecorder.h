...
@interface User_Onrspqryexecorder : BaseResponseModel
...
@property(nonatomic,strong) NSNumber *ActionType;//执行类型    '1'->执行;'2'->放弃;
@property(nonatomic,strong) NSNumber *PosiDirection;//保留头寸申请的持仓方向    '1'->净;'2'->多头;'3'->空头;
@property(nonatomic,strong) NSNumber *ReservePositionFlag;//期权行权后是否保留期货头寸的标记,该字段已废弃    '0'->保留;'1'->不保留;
@property(nonatomic,strong) NSNumber *CloseFlag;//期权行权后生成的头寸是否自动平仓    '0'->自动平仓;'1'->免于自动平仓;
...
@property(nonatomic,strong) NSNumber *OrderSubmitStatus;//执行宣告提交状态    '0'->已经提交;'1'->撤单已经提交;'2'->修改已经提交;'3'->已经接受;'4'->报单已经被拒绝;'5'->撤单已经被拒绝;'6'->改单已经被拒绝;
...
@property(nonatomic,strong) NSNumber *ExecResult;//执行结果    'n'->没有执行;'c'->已经取消;'0'->执行成功;'1'->期权持仓不够;'2'->资金不够;'3'->会员不存在;'4'->客户不存在;'6'->合约不存在;'7'->没有执行权限;'8'->不合理的数量;'9'->没有足够的历史成交;'a'->未知;
