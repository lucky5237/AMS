...
@interface User_Onrspqryorder : BaseResponseModel
...
@property(nonatomic,strong) NSNumber *OrderPriceType;//报单价格条件    '1'->任意价;'2'->限价;'3'->最优价;'4'->最新价;'5'->最新价浮动上浮1个ticks;'6'->最新价浮动上浮2个ticks;'7'->最新价浮动上浮3个ticks;'8'->卖一价;'9'->卖一价浮动上浮1个ticks;'A'->卖一价浮动上浮2个ticks;'B'->卖一价浮动上浮3个ticks;'C'->买一价;'D'->买一价浮动上浮1个ticks;'E'->买一价浮动上浮2个ticks;'F'->买一价浮动上浮3个ticks;'G'->五档价;
...
@property(nonatomic,strong) NSNumber *TimeCondition;//有效期类型    '1'->立即完成，否则撤销;'2'->本节有效;'3'->当日有效;'4'->指定日期前有效;'5'->撤销前有效;'6'->集合竞价有效;
@property(nonatomic,copy) NSString *GTDDate;//GTD日期
@property(nonatomic,strong) NSNumber *VolumeCondition;//成交量类型    '1'->任何数量;'2'->最小数量;'3'->全部数量;
@property(nonatomic,strong) NSNumber *MinVolume;//最小成交量
@property(nonatomic,strong) NSNumber *ContingentCondition;//触发条件    '1'->立即;'2'->止损;'3'->止赢;'4'->预埋单;'5'->最新价大于条件价;'6'->最新价大于等于条件价;'7'->最新价小于条件价;'8'->最新价小于等于条件价;'9'->卖一价大于条件价;'A'->卖一价大于等于条件价;'B'->卖一价小于条件价;'C'->卖一价小于等于条件价;'D'->买一价大于条件价;'E'->买一价大于等于条件价;'F'->买一价小于条件价;'H'->买一价小于等于条件价;
@property(nonatomic,strong) NSNumber *StopPrice;//止损价
@property(nonatomic,strong) NSNumber *ForceCloseReason;//强平原因    '0'->非强平;'1'->资金不足;'2'->客户超仓;'3'->会员超仓;'4'->持仓非整数倍;'5'->违规;'6'->其它;'7'->自然人临近交割;
...
@property(nonatomic,strong) NSNumber *OrderSubmitStatus;//报单提交状态    '0'->已经提交;'1'->撤单已经提交;'2'->修改已经提交;'3'->已经接受;'4'->报单已经被拒绝;'5'->撤单已经被拒绝;'6'->改单已经被拒绝;
...
@property(nonatomic,strong) NSNumber *OrderSource;//报单来源    '0'->来自参与者;'1'->来自管理员;
@property(nonatomic,strong) NSNumber *OrderStatus;//报单状态    '0'->全部成交;'1'->部分成交还在队列中;'2'->部分成交不在队列中;'3'->未成交还在队列中;'4'->未成交不在队列中;'5'->撤单;'a'->未知;'b'->尚未触发;'c'->已触发;
@property(nonatomic,strong) NSNumber *OrderType;//报单类型    '0'->正常;'1'->报价衍生;'2'->组合衍生;'3'->组合报单;'4'->条件单;'5'->互换单;
