...
@interface User_Onrspqryquote : BaseResponseModel
...
@property(nonatomic,strong) NSNumber *OrderSubmitStatus;//报价提交状态    '0'->已经提交;'1'->撤单已经提交;'2'->修改已经提交;'3'->已经接受;'4'->报单已经被拒绝;'5'->撤单已经被拒绝;'6'->改单已经被拒绝;
...
@property(nonatomic,strong) NSNumber *QuoteStatus;//报价状态    '0'->全部成交;'1'->部分成交还在队列中;'2'->部分成交不在队列中;'3'->未成交还在队列中;'4'->未成交不在队列中;'5'->撤单;'a'->未知;'b'->尚未触发;'c'->已触发;
